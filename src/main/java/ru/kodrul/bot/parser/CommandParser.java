...
public class CommandParser {
    ...
    public CommandArguments parseCommandWithQuotes(String fullText) {
        ...
        try {
            Matcher matcher = COMMAND_PATTERN.matcher(fullText);
            if (!matcher.find()) {
                log.warn("Failed to parse command: {}", fullText);
                return args; // вернет null значения
            }

            // Извлекаем группы
            args.setChatId(Long.parseLong(matcher.group(1)));
            args.setGroupName(matcher.group(2));
            args.setSchedule(matcher.group(3));

            String messagePart = matcher.group(4).trim();
            ...
            log.debug("Parsed command: chatId={}, group={}, schedule='{}', message='{}', imageUrl={}",
                    args.getChatId(), args.getGroupName(), args.getSchedule(),
